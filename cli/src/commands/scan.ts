...
import type { ScanCandidate, Severity } from "../schema.js";

/** 検出ルール。行単位の正規表現で候補を生成する。 */
interface ScanRule {
  readonly id: string;
  /** 対応すると推定される綱領項目ID */
...
function scanFile(root: string, absPath: string): ScanCandidate[] {
...
/** scan サブコマンドの本体。 */
export function runScan(root: string, format: "json" | "text"): string {
