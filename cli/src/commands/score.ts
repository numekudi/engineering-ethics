...
export const PRINCIPLE_WEIGHT: Readonly<Record<number, number>> = {
  1: 0.22, // 公共性
  2: 0.1, // 顧客ならびに雇用者
  3: 0.18, // 製品
  4: 0.12, // 判断
...
function principleOf(finding: Finding): number {
...
function decideVerdict(overall: number, hasCritical: boolean): Verdict {
  if (hasCritical) {
    // critical があれば合格は不可。総合が要改善域なら要改善、それ未満は不適合。
    return overall >= REVIEW_THRESHOLD ? "要改善" : "不適合";
  }
  if (overall >= PASS_THRESHOLD) return "合格";
  if (overall >= REVIEW_THRESHOLD) return "要改善";
  return "不適合";
}
 ...
 */
export function aggregate(findings: readonly Finding[], ethics: CodeOfEthics): AuditResult {
  const confirmed = findings.filter((f) => f.confirmed);

  const principles: PrincipleScore[] = ethics.principles.map((p) => {
  ...
  const weightedSum = principles.reduce(
    (sum, ps) => sum + ps.score * (PRINCIPLE_WEIGHT[ps.principle] ?? 0),
    0,
  );
  const weightTotal = principles.reduce(
    (sum, ps) => sum + (PRINCIPLE_WEIGHT[ps.principle] ?? 0),
    0,
  );
