...
export interface TweetDraft {
  ...
  /** X の intent URL（開くと本文入りの投稿画面が出る） */
  readonly url: string;
}

/** 判定に対応する絵文字（視認性のため）。 */
function verdictEmoji(result: AuditResult): string {
...
function worstPrinciple(result: AuditResult): AuditResult["principles"][number] | undefined {
  if (result.principles.length === 0) return undefined;
  return [...result.principles].sort(
    (a, b) => a.score - b.score || a.principle - b.principle,
  )[0];
}

/** AuditResult からツイート本文と intent URL を組み立てる純粋関数。 */
export function buildTweet(result: AuditResult, input: TweetInput = {}): TweetDraft {
...
export function formatTweet(draft: TweetDraft, format: "json" | "text"): string {
