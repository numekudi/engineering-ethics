...
function requireRecord(value: unknown, path: string): Record<string, unknown> {
  if (!isRecord(value)) throw new ValidationError(path, "オブジェクトが必要です");
  return value;
}

function requireArray(value: unknown, path: string): readonly unknown[] {
  if (!Array.isArray(value)) throw new ValidationError(path, "配列が必要です");
  return value;
}

function requireString(value: unknown, path: string): string {
  if (typeof value !== "string") throw new ValidationError(path, "文字列が必要です");
  return value;
}

function requireNonEmptyString(value: unknown, path: string): string {
  const s = requireString(value, path);
  if (s.length === 0) throw new ValidationError(path, "空文字は不可です");
  return s;
}
...
function requireBoolean(value: unknown, path: string): boolean {
  if (typeof value !== "boolean") throw new ValidationError(path, "真偽値が必要です");
  return value;
}

function requireSeverity(value: unknown, path: string): Severity {
  const s = requireString(value, path);
...
function requireItemId(value: unknown, path: string): string {
  const s = requireString(value, path);
  if (!/^[1-8]\.\d{2}$/.test(s)) {
...
function parsePrinciple(value: unknown, path: string): Principle {
...
function parseEthicsItem(value: unknown, path: string): EthicsItem {
  const o = requireRecord(value, path);
  ...
  return {
    ...
    redFlags: redFlagsRaw.map((f, i) =>
      requireNonEmptyString(f, `${path}.redFlags[${i}]`),
    ),
  };
}

/** data/se-code.json をパースし CodeOfEthics として検証する。 */
export function parseCodeOfEthics(value: unknown): CodeOfEthics {
...
const VERDICTS: readonly Verdict[] = ["合格", "要改善", "不適合"];

function requireVerdict(value: unknown, path: string): Verdict {
  const s = requireString(value, path);
...
function parsePrincipleScore(value: unknown, path: string): PrincipleScore {
  const o = requireRecord(value, path);
  return {
    principle: requireNumber(o["principle"], `${path}.principle`),
    name: requireNonEmptyString(o["name"], `${path}.name`),
