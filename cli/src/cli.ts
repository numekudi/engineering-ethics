...
interface Flags {
  ...
  readonly [key: string]: string | boolean | readonly string[] | undefined;
}
...
function resolveFormat(flags: Flags): "json" | "text" {
  const f = strFlag(flags, "format", "f");
  if (f === undefined || f === "json") return "json";
...
function readInput(flags: Flags): string {
...
function dispatch(flags: Flags): string {
  ...
  switch (command) {
    ...
    case "scan": {
      ...
      if (root === undefined) throw new CliError("scan には <repo-path> が必要です", "NO_PATH");
